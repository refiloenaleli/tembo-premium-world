    ...
    <div className="space-y-4">
      ...
      {orders && orders.length > 0 ? (
        <div className="space-y-3">
          {orders.map((order) => {
            // Use customer's phone if available, otherwise fall back to admin number
            ...
            return (
              <div key={order.id} className="bg-card border border-border rounded-lg p-4">
                ...
                <div className="flex flex-wrap gap-2 border-t border-border pt-3">
                  {/* FIXED: Now messages the CUSTOMER */}
                  <a
                    href={`https://wa.me/${customerPhone}?text=${encodeURIComponent(`Hi ${order.customer_name}, regarding your Tembo order #${order.id.slice(0,8)} (${order.currency} ${order.total_amount}). `)}`}
                    target="_blank"
                  ...
                  {/* Email already worked correctly */}
                  <a
